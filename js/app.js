 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            ...
            let largeSliders = document.querySelectorAll(".swiper-card");
            let prevArrow = document.querySelectorAll(".prev");
            let nextArrow = document.querySelectorAll(".next");
            if (largeSliders) largeSliders.forEach(((slider, index) => {
                slider.children[0].children.length;
                new core(slider, {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
...
***************************************************************************** */
        ...
        const scriptImages = document.querySelector(".image__vidget");
        const imageClouse = document.querySelector(".clouse__image");
        scriptImages.hidden = false;
        vidget.addEventListener("click", (function(e) {
            let target = e.target;
            social.classList.toggle("vidjet-active");
            scriptImages.classList.toggle("vidget-hover");
            if (target.closest(".image__vidget")) {
                target.closest(".image__vidget").hidden = true;
                imageClouse.hidden = false;
            }
            if (target.closest(".clouse__image")) {
                target.closest(".clouse__image").hidden = true;
                scriptImages.hidden = false;
            }
        }));
        function shineLinks(id) {
            try {
                const nav = document.getElementById("menu").querySelectorAll(".menu-item");
                const url = document.location.href;
                for (let i = 0; i < nav.length; i++) if (url == nav[i].href) nav[i].className += " menu-act";
            } catch (e) {}
        }
        shineLinks("menu-act");
        window["FLS"] = false;
